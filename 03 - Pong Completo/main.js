...
function atualizaAnimacao(){
    ...
    // barra esquerda (x = -0.9)
    if (txBola - 0.05 < -0.85 && Math.abs(tyBola - tyBE) < 0.25) {
        txBola_offset = Math.abs(txBola_offset);
    }

    // barra direita (x = 0.9)
    if (txBola + 0.05 > 0.85 && Math.abs(tyBola - tyBD) < 0.25) {
        txBola_offset = -Math.abs(txBola_offset);
    }

    // a bola saiu pela lateral esquerda: ponto da direita
    if (txBola < -1.0) {
        pontosBD += 1;
        placar.textContent = pontosBE + " x " + pontosBD;
        txBola = 0.0;
        tyBola = 0.0;
    }

    // a bola saiu pela lateral direita: ponto da esquerda
    if (txBola > 1.0) {
        pontosBE += 1;
        placar.textContent = pontosBE + " x " + pontosBD;
        txBola = 0.0;
    ...
    const limite = 1.0 - 0.2;

    tyBE = Math.max(-limite, Math.min(limite, tyBE));
    tyBD = Math.max(-limite, Math.min(limite, tyBD));
